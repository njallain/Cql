...
class FullMigrationTests: XCTestCase {
	...
	func testMigrate() {
		do {
			...
			let projects = try conn.find(Predicate.any(Project.self))
			...
			let items = try conn.find(Predicate.any(Item.self))
			...
			let projectItems = try conn.find(Predicate.any(ProjectItem.self))
			...
			let comments = try conn.find(Predicate.any(Comment.self))
			...
			let attachments = try conn.find(Predicate.any(Attachment.self))
